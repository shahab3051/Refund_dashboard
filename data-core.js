...
/* ===================================================================
   ...
   fetches the current dataset from the Google Apps Script API defined
   in CONFIG.DATA_SOURCE_URL (see config.js). The Apps Script endpoint
   returns the exact same {meta, dims, fact} shape that the in-browser
   Excel importer (data-upload.js) already produces, so every existing
   render / filter / drill-through function keeps working unchanged.
   ...
   recent data if the network / API is temporarily unavailable.
   =================================================================== */
...
/**
 * Fetches the current dataset from the Apps Script Web App configured in
 * config.js. Falls back to the last locally-cached copy if the request
